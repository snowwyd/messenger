...
export default function App() {
    ...
    const selectSignUpButton = () =>
        hoverEffect(signUpButtonRef.current, signUpTextRef.current, selectionSignUpRef.current, false);
    const deselectSignUpButton = () =>
        hoverEffect(signUpButtonRef.current, signUpTextRef.current, selectionSignUpRef.current, true);
    const selectSignInButton = () =>
        hoverEffect(signInButtonRef.current, signInTextRef.current, selectionSignInRef.current, false);
    const deselectSignInButton = () =>
        hoverEffect(signInButtonRef.current, signInTextRef.current, selectionSignInRef.current, true);
    ...
    function switchForm(toLeft) {
        ...
        const animateImageBlock = (progress) => {
            ...
            signInButtonRef.current.style.right = toLeft
                ? progress * -btnsInterval + 'px'
                : -btnsInterval + progress * btnsInterval + 'px';
            signUpButtonRef.current.style.left = toLeft
                ? -btnsInterval + progress * btnsInterval + 'px'
                : progress * -btnsInterval + 'px';

            registerFormRef.current.style.right = toLeft
                ? progress * -formsInterval + 'px'
                : -formsInterval + progress * formsInterval + 'px';
            loginFormRef.current.style.left = toLeft
                ? -formsInterval + progress * formsInterval + 'px'
                : progress * -formsInterval + 'px';
        };
